...
- When the command completes, the MCP server shall record the step in the session log with the phase, command, exit code, duration, and timestamp (completion time of the command, used for sorting attempts and deduplication).
...
- The `StepRecord` type shall include phase, command, source ("mcp" | "npm-instrumentation"), exitCode, durationMs, timestamp (ISO 8601 string representing command completion time — used for sorting attempts and for the gateway's timestamp-based deduplication), and outputSummary (`string | null`, represented as `null` in JSON when no summary is available).
...
- The MCP server composition root (`src/mcp-server.ts`) shall generate a UUID session ID on startup using `crypto.randomUUID()` and retain it in memory for subsequent tool calls. UUID generation is an infrastructure concern and must not be performed in entities or use cases.
- On the first tool call that provides a `targetDir`, the MCP server shall write the active-session marker via the `SessionLogGateway` to `.lousy-agents/active-session.json` within that directory, and initialize the `session.json` with project metadata (name from package.json, packageManager) and `mandatoryPhases` (from `discover_feedback_loops`). Both the marker and session log creation are deferred until this point because `targetDir` is not known until the agent makes a tool call.
