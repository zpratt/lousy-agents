...
- `packages/agent-shell/src/copilot-enhance.ts` — New module: optional AI-enhanced analysis via `@github/copilot-sdk` with lousy-agents MCP server integration
- `packages/agent-shell/src/copilot-prompt.ts` — Socratic prompt template for AI-enhanced analysis
...
- `@lousy-agents/mcp` (runtime — connected as local MCP server via copilot SDK for structured project discovery)
- `zod` (existing)
...
flowchart TB
    ...
    subgraph Enhance["Copilot Enhancement (optional, advisory only)"]
...
sequenceDiagram
    ...
    participant MCP as lousy-agents MCP
    ...
    alt SDK available
        SDK->>MCP: discover_feedback_loops, discover_environment
        MCP-->>SDK: Structured SDLC discovery results
        SDK-->>CLI: Enhanced suggestions (advisory only)
...
**Requirements**:
- Generates allow list from discovered commands as exact-match rules (reserving wildcard patterns only for predefined default-safe commands)
