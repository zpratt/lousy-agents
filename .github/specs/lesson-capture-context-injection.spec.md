...
- If a `--files` path resolves outside the current working directory, then the context command shall drop that path with a warning and continue processing remaining valid paths (fail-open per-path). The command exits non-zero only when all provided `--files` paths resolve outside the workspace or are blank; in that case it still emits a valid `hookSpecificOutput` envelope. The containment check must be boundary-safe — see **Design § Path normalization requirement** for the required algorithm.
...
- **Injection**: PreToolUse hook for Edit/Write calls the fixed command `lousy-agents context` and passes Claude Code hook input JSON on stdin. Manual/debug invocations may pass a single comma-separated `--files` flag. SessionStart hook injects invariants at session open.
...
4. Otherwise, if stdin is empty and no `--files` flag is provided, the CLI treats the invocation as the SessionStart invariant-injection path (`hookEventName: "SessionStart"`).
...
- [x] ~~Should `lousy-agents context --files` accept comma-separated paths, repeated flags, or both?~~ **Resolved**: `--files` accepts a single comma-separated string (e.g., `--files path1,path2`). This matches the `citty` string-flag pattern used throughout the CLI and aligns with DESIGN.md. Repeated-flag syntax is not supported.
...
- Accepts one or more file paths via a single comma-separated `--files` flag (e.g., `--files path1,path2`) for manual/debug invocations.
