...
- While hooks.json already exists but is missing the flight recording hook, the system shall prompt the user: "Enable flight recording to capture all agent tool usage?" with a default of "yes."
- While hooks.json already exists but is missing the policy-check hook, the system shall prompt the user: "Enable policy-based command blocking?" with a default of "yes."
...
- If stdin is not valid JSON, then the handler shall write a diagnostic to stderr and exit gracefully without emitting telemetry
