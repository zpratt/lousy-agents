...
describe("scanProject", () => {
    ...
    describe("given workflow YAML with backslash line continuations in literal blocks", () => {
        it("should concatenate continued lines into a single command", async () => {
            const workflowsDir = join(testDir, ".github", "workflows");
            await mkdir(workflowsDir, { recursive: true });
            await writeFile(
                join(workflowsDir, "deploy.yml"),
        ...
        it("should handle mixed continued and standalone lines", async () => {
            const workflowsDir = join(testDir, ".github", "workflows");
            await mkdir(workflowsDir, { recursive: true });
            await writeFile(
                join(workflowsDir, "ci.yml"),
        ...
        it("should detect block scalars with inline comments after the indicator", async () => {
            const workflowsDir = join(testDir, ".github", "workflows");
            await mkdir(workflowsDir, { recursive: true });
            await writeFile(
                join(workflowsDir, "ci.yml"),
        ...
        it("should detect folded block scalars with inline comments after the indicator", async () => {
            const workflowsDir = join(testDir, ".github", "workflows");
            await mkdir(workflowsDir, { recursive: true });
            await writeFile(
                join(workflowsDir, "ci.yml"),
    ...
    describe("given workflow YAML with inline comments on run commands", () => {
        ...
        it("should strip inline comments from multi-line literal block commands", async () => {
            const workflowsDir = join(testDir, ".github", "workflows");
            await mkdir(workflowsDir, { recursive: true });
            await writeFile(
                join(workflowsDir, "ci.yml"),
            ...
            expect(result.workflowCommands).toContainEqual("npm test");
            expect(
                result.workflowCommands.some((cmd) =>
                    cmd.includes("# install"),
                ),
            ).toBe(false);
            expect(
                result.workflowCommands.some((cmd) => cmd.includes("# run")),
            ).toBe(false);
        });

        it("should preserve hash symbols in arguments within block commands", async () => {
            const workflowsDir = join(testDir, ".github", "workflows");
            await mkdir(workflowsDir, { recursive: true });
            await writeFile(
                join(workflowsDir, "ci.yml"),
        ...
        it("should strip trailing comment while preserving hash in arguments within block commands", async () => {
            const workflowsDir = join(testDir, ".github", "workflows");
            await mkdir(workflowsDir, { recursive: true });
            await writeFile(
                join(workflowsDir, "ci.yml"),
                [
                    ...
                    "    steps:",
                    "      - run: |",
                    "          npm run build -- --tag=#latest # build image",
                ].join("\n"),
            );
    ...
    describe("given a mise.toml with multi-line task run commands", () => {
        it("should extract the first line as the command", async () => {
            ...
            expect(result.miseTasks[0]?.command).toBe("npm run lint");
        });
    });
});
