...
import type { LessonContextUseCase } from "@lousy-agents/core/use-cases/lesson-context-use-case.js";
...
async function readFileContents(
    ...
    for (const fp of filePaths) {
        try {
            ...
            await resolveSafePath(rootDir, fp);
            const text = await readTextWithinRoot(
                rootDir,
                fp,
...
export function createContextCommand(useCase: LessonContextUseCase) {
    return defineCommand({
        ...
        run: async (context: CommandContext) => {
            ...
            const rawFiles =
                typeof context.args?.files === "string"
                    ? context.args.files
                    : "";
